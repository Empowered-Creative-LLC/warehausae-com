---
id: 087af572-1cd3-40f7-a998-ab4f3e0000a2
blueprint: project
title: 4844 Market Street Self-Storage
slug: 4844-market-street-self-storage
source_url: https://warehausae.com/project/4844-market-street-self-storage/
location: Jackson Township, York County, PA
lead_heading: Site Planning Makes Way for Moove-In Storage
intro_prose: >-
  Through strategic site planning, Warehaus’ civil engineering team effectively addressed the demand for increased storage at Moove In Storage’s facility at 4844 West Market Street in Jackson
  Township, York County, PA. This convenient location, accessible from Route 30, allowed for the construction of four storage buildings, specifically designed to cater to the growing need for secure
  and affordable self-storage units in the area.
narrative:
  we_listen: >-
    The comprehensive project program embraced various aspects, including surveying, wetland and stream investigation, site development planning, erosion control planning, and construction phase
    services. As part of the meticulous process, the team conducted an as-built survey to ensure strict alignment with the approved plans.
  we_design: >-
    Recognizing the evolving regulatory landscape, Warehaus provided design assistance for a previously approved plan, making minor adjustments necessary to accommodate slight variances between the
    original design and the actual construction.
  we_deliver: >-
    The project concluded in February 2023, underscoring a significant milestone in Moove In Storage’s unwavering commitment to delivering convenient and reliable storage options to the residents of
    York, PA.
client: Investment Real Estate, LLC
size: Undisclosed
completion_date: February 2023
services_provided:
  - label: Civil
    url: https://warehausae.com/service/civil/
industries:
  - label: Retail and Hospitality
    url: https://warehausae.com/industry/retail-and-hospitality/
hero_image_url: /assets/imported/2022/03/Asset-1.png
gallery_images:
  - url: /assets/imported/2024/04/Moove-In-Self-Storage-8-700x700.webp
    alt: Civil engineering design for Moove In Storage facility
  - url: /assets/imported/2024/04/Moove-In-Self-Storage-15-700x700.webp
    alt: Civil engineering design for Moove In Storage facility
  - url: /assets/imported/2024/04/Moove-In-Self-Storage-17-700x700.webp
    alt: Civil engineering design for Moove In Storage facility
  - url: /assets/imported/2024/04/Moove-In-Self-Storage-15-2-768x400.webp
    alt: Moove-In Self Storage-15
related_projects:
  - title: Ballpark Commons
    url: https://warehausae.com/project/ballpark-commons/
    categories:
      - Architecture
      - Civil
      - Interiors
  - title: Zion Lutheran Church Adaptive Reuse
    url: https://warehausae.com/project/zion-lutheran-church-adaptive-reuse/
    categories:
      - Architecture
      - Historic
  - title: Susquehanna Trail Dunkin’ & Jimmy John’s
    url: https://warehausae.com/project/susquehanna-trail-dunkin-jimmy-johns/
    categories:
      - Civil
      - Civil Engineering
  - title: McHenry Row Office and Courtyard by Marriott Hotel
    url: https://warehausae.com/project/mchenry-row-office-hotel/
    categories:
      - Architecture
      - Interiors
  - title: Blackworth Live Fire Grill
    url: https://warehausae.com/project/blackworth-live-fire-grill/
    categories:
      - Architecture
      - Interiors
  - title: Courtyard by Marriott at Rowan Boulevard
    url: https://warehausae.com/project/courtyard-by-marriott-rowan-boulevard/
    categories:
      - Architecture
  - title: Hilton Garden Inn
    url: https://warehausae.com/project/hilton-garden-inn/
    categories:
      - Architecture
      - Civil
  - title: Inn at Wyndridge Farm
    url: https://warehausae.com/project/inn-at-wyndridge-farm/
    categories:
      - Interiors
  - title: Wyndridge Farm
    url: https://warehausae.com/project/wyndridge-farm/
    categories:
      - Civil
      - Interiors
  - title: Dunkin® East
    url: https://warehausae.com/project/dunkin-east-retail/
    categories:
      - Civil
  - title: Retail and Commercial
    url: https://warehausae.com/project/retail-and-commercial/
    categories:
      - Architecture
      - Civil
  - title: Hershey Tanger Outlets Site and Facade Improvements
    url: https://warehausae.com/project/hershey-tanger-outlets-site-and-facade-improvements/
    categories:
      - Architecture
      - Civil
      - Structural
  - title: The Barn Resort
    url: https://warehausae.com/project/the-barn-resort/
    categories:
      - Architecture
      - Civil
      - Structural
  - title: Test Fit Capabilities
    url: https://warehausae.com/project/test-fits/
    categories:
      - Architecture
      - Interiors
  - title: Italian Steakhouse
    url: https://warehausae.com/project/italian-steakhouse/
    categories:
      - Architecture
      - Interiors
is_test_fits_subpage: false
seo_title: 4844 Market Street Self-Storage - Warehaus
seo_description: Explore the 4844 Market Street Self-Storage project by Warehaus, focusing on strategic site planning in Jackson Township, York County, PA.
seo_og_image_url: /assets/imported/2022/03/Asset-1.png
---
