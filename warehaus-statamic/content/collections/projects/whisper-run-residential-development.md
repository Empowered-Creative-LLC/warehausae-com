---
id: 8675585f-e9ca-4a5b-903b-9b274f3d9957
blueprint: project
title: Whisper Run Residential Development
slug: whisper-run-residential-development
source_url: https://warehausae.com/project/whisper-run-residential-development/
location: East York, PA
lead_heading: Partnering on the evolving needs of a residential development
intro_prose: Warehaus’ involvement with Whisper Run, a 100-lot residential development, dates back to 2007, when the civil engineering team assisted the client in securing an NPDES renewal.
narrative:
  we_listen: >-
    Over the years and through two project phases, Warehaus collaborated on various aspects of site, including retrofitting the development’s design to meet evolving requirements, preparing security
    bond reductions, and submitting NPDES minor amendments on an individual lot basis. Rain gardens with underground storage were also implemented on individual lots.
  we_design: >-
    Warehaus’ comprehensive services also included the completion of as-builts for the sanitary sewer system and collaboration with the municipality to facilitate the acceptance of roads within the
    development.
  we_deliver: >-
    This project showcases the team’s commitment to navigating regulatory challenges, adapting to evolving requirements, and providing end-to-end solutions for successful residential subdivision
    development.
client: Argento Family Partners
size: 31.5 acres
completion_date: "2024"
services_provided:
  - label: Civil
    url: /services/civil_engineering/
industries:
  - label: Multi-Family
    url: /multi-family/
hero_image_url: /assets/imported/2024/09/Whisper-Run.jpg
gallery_images:
  - url: /assets/imported/2024/09/IMG_3926-700x700.jpg
    alt: img_3926
  - url: /assets/imported/2024/09/IMG_3934-700x700.jpg
    alt: img_3934
  - url: /assets/imported/2024/09/1-700x700.jpg
    alt: "1"
  - url: /assets/imported/2024/09/Whisper-Run-700x700.jpg
    alt: whisper-run
  - url: /assets/imported/2024/09/Whisper-Run_09052024_0014-700x700.webp
    alt: Civil Engineering at Whisper Run Residential Development
  - url: /assets/imported/2024/09/pro-qp0TUXLY-700x700.jpg
    alt: Whisper Run Residential Development Civil Engineering Rain Garden
related_projects:
  - title: 800 South Queen Street Multi-Family Apartments
    url: /project/800-south-queen-street-multi-family-apartments/
    categories:
      - Architecture
    image_url: /assets/imported/2024/01/800-S-Queen-view-BRANDED-700x700.jpg
  - title: Fox Apartments
    url: /project/fox-apartments/
    categories:
      - Architecture
      - Structural
    image_url: /assets/imported/2024/11/Fox-Apartments-10302024-27-700x700.webp
  - title: 205 State Street Apartments
    url: /project/205-state-street-apartments/
    categories:
      - Architecture
    image_url: /assets/imported/2024/07/state-street-apartments-06192024-5-768x400.webp
  - title: Nelson Kohl Apartments
    url: /project/nelson-kohl-apartments/
    categories:
      - Architecture
      - Interiors
    image_url: /assets/imported/2023/03/Nelson-Kohl-10_2018-0001-700x501.jpg
  - title: 12th and Macdonald Apartments
    url: /project/12th_and_macdonald/
    categories:
      - Architecture
    image_url: /assets/imported/2023/03/CORNER.jpg
  - title: Franklin Square Multi-Family Mixed Use
    url: /project/franklin-square-multi-family-mixed-use/
    categories:
      - Architecture
    image_url: /assets/imported/2024/05/Franklin-Square-FEATURE-700x700.webp
  - title: The Flats at Hampden
    url: /project/flats-at-hampden-apartments/
    categories:
      - Architecture
    image_url: /assets/imported/2024/05/Flats-at-Hampden-18-700x588.webp
  - title: The Steel Works Mixed-Use Development
    url: /project/the-steel-works-mixed-use-development/
    categories:
      - Architecture
    image_url: /assets/imported/2023/03/Steelworks-2.jpg
  - title: Union Flats Apartments
    url: /project/union-flats-apartments/
    categories:
      - Architecture
    image_url: /assets/imported/2023/03/Union-Flats-131082.jpg
  - title: University Lofts
    url: /project/university-lofts/
    categories:
      - Architecture
      - Historic
    image_url: /assets/imported/2023/03/University-Lofts_Rendering-052419_00024-700x700.jpg
  - title: 3400 Boston Street Apartments
    url: /project/3400-boston-street/
    categories:
      - Architecture
    image_url: /assets/imported/2023/05/3400-Boston-Street_Rendering-20190521_0004.jpg
  - title: Sycamore Homes
    url: /project/sycamore-homes/
    categories:
      - Architecture
    image_url: /assets/imported/2023/10/2023-04-28-Sycamore-Homes-Updated-Perspective-VE-Options-700x700.webp
is_test_fits_subpage: false
seo_title: Whisper Run Residential Development - Warehaus
seo_description: Discover the Whisper Run Residential Development project by Warehaus.
seo_og_image_url: /assets/imported/2024/09/Whisper-Run.jpg
---
